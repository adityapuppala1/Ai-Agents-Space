...
- **One command to run it: `npx agentspace`.** A launcher (`bin/agentspace.js`) finds a free port, keeps the database in the operating system's own per-user data directory rather than inside the downloaded package, opens the browser, and hands over to the server. Provider detection and session observation already run on startup, so it shows the assistants on the machine without being configured. Verified end to end from a packed tarball: only `ws` is installed, and the office drew a real live Claude Code session. **Not published** — the name is available and the package is prepared, but publishing and choosing a licence are the owner's decisions.
